...
test(
  'cli edit: custom device types can enable clusters from the base data model',
  async () => {
    // Device types declared in custom XML reference clusters that live in the
    // primary zcl-properties package. Cluster enable/list must still resolve
    // those base clusters on the custom-device endpoint.
    let file = path.join(workDir, 'cli-edit-custom-device-type.zap')
    repoScratchFiles.push(file)
    let matter = { ...matterPackages, zapFile: file }
    let xml = path.resolve(
      'test/resource/townhouse/townhouse-energy-devices.xml'
    )
    ...
    let rows = JSON.parse(listed.output).operations[0].rows
    expect(rows.length).toBeGreaterThan(0)
    expect(rows.some((r) => r.name === 'Identify')).toBe(true)
    ...
    expect(mode.errors).not.toContain('Unknown cluster')
  },
  testUtil.timeout.long()
)
