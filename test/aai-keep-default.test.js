...
// FeatureMap is the canonical case: it is answered by the attribute access
// interface, so it needs no room in the attribute store, but its value is a
// fixed bitmap that the implementation can read out of the generated defaults.
...
test('keepDefault asks for the storage policy that keeps the default value', () => {
  let normalized = zclLoaderSilabs.normalizeAttributeAccessInterfaceAttributes({
    'Access Control': [
      'ClusterRevision',
  ...
  expect(normalized['Access Control']).toEqual([
    ...
    {
      name: 'FeatureMap',
      storagePolicy: dbEnum.storagePolicy.attributeAccessInterfaceWithDefault
    },
    {
      name: 'AccessControlEntriesPerFabric',
      storagePolicy: dbEnum.storagePolicy.attributeAccessInterface
...
test(
  'Both attribute access interface policies are forced to external storage',
  async () => {
    expect(
      await matterSdk.computeStorageOptionNewConfig(
...
test(
  'A cluster/attribute pair that keeps its default wins over a plain entry',
  async () => {
    ...
    // built in Matter metadata, so both policies match it, and the one that
    // keeps the default has to win.
    await queryPackage.insertOptionsKeyValues(
      ...
      [
        {
          code: dbEnum.storagePolicy.attributeAccessInterfaceWithDefault,
    ...
    expect(
      matterSdk.forcedExternalStoragePolicy(
        forcedExternal,
        keepDefaultCluster,
        keepDefaultAttribute
      )
    ).toEqual(dbEnum.storagePolicy.attributeAccessInterfaceWithDefault)
